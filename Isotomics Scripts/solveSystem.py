...
def GJElim(Matrix, identityInversion = [], storeRank = 0, augMatrix = False, AugAmount = 1, store = False, sanitize = False, eps = 10**-8):
    '''
    ...
    ident = np.identity(rows)

    if identityInversion != [] and augMatrix == True:
        M = np.matmul(identityInversion, M)
        return M, storeRank, ident
    ...
    while r < rows and c < colLimit:
        ...
        if True in (np.abs(M[r:,c]) > eps):
            ...
            ident[[r, pivotRow]] = ident[[pivotRow, r]] 

            divisor = M[r,c].copy()
            M[r] = M[r]/ divisor
            ident[r] = ident[r]/ divisor

            for i in range(1,rows-r):
                factor = (M[r+i,c]/M[r,c]).copy()

                M[r+i] -= (factor * M[r])
                ident[r+i] -= (factor * ident[r])


            for j in range(0,r):
                factor = (M[j,c]/M[r,c]).copy()
                M[j] -= factor * M[r]
                ident[j] -= factor * ident[r]
...
def MonteCarloMN(MNKey, Isotopologues, standardData, sampleData, OCorrection, 
    ...
    for i in tqdm(range(N), disable = disableProgress):
        ...
        if i == 0:
            solve = GJElim(AugMatrix, augMatrix = True)
            identInversion = copy.deepcopy(solve[2])
            storeRank = solve[1]
            res[MNKey]['Inversion'] = identInversion

        else:
            solve = GJElim(AugMatrix, identityInversion = identInversion, storeRank = storeRank, augMatrix = True)
