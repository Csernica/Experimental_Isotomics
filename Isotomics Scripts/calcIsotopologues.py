...
def calculateIsotopologueConcentrations(setOfAllIsotopologues, symmetryNumbers, concentrationArray, disable = False):
    ...
    for i, isotopologue in enumerate(tqdm(setOfAllIsotopologues, disable = disable)):
        ...
        for index, value in enumerate(isotopologue):
            #by default we store isotopologues as tuples; but if they are read in from somewhere, e.g. json, they may be a list
        ...
        d[string] = {'Conc':isotopologueConc * number,'num':number,'Full':isotopologue}
...
def pullOutIsotopologues(MNDict):
    '''
    One option to improve computational speed; you can calculate isotopologues once, pull out a certain number
    into an MNDict (i.e. by mass), then store them using this function. The stored can be written to .json
    and imported as an 'override Dict' for inputToAtomDict, below, and only the selected isotopologues
    will be calculated. 
    '''
    storedIsotopologues = {'bigAUnderThreshold':[],
                            'SNUnderThreshold':[]}

    for MN, MNData in MNDict.items():
        for iso, isoData in MNData.items():

            storedIsotopologues['bigAUnderThreshold'].append(isoData['Full'])
            storedIsotopologues['SNUnderThreshold'].append(int(isoData['Number']))

    return storedIsotopologues

def inputToAtomDict(molecularDataFrame, overrideIsoDict = {}, disable = False, M1Only = False):
    '''
    ...
    Inputs:
        molecularDataFrame: A dataFrame containing information about the molecule.
        overrideIsoDict: To avoid recalculating the entire isotopologue dictionary each time, the user may specify a "overrideIsoDict". This is a .json file yielded by the 'pullOutIsotopologues' function. It contains lists specifying each isotopologue and the identity numbers for each isotoplogue. 
    ...
    if overrideIsoDict != {}:
        bigA, SN = overrideIsoDict['bigAUnderThreshold'], overrideIsoDict['SNUnderThreshold']
